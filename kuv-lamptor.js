...
.download-card__stats .eta {
  ...
  font-size: inherit;
    ...
    <div class="download-circle__down-arrow">\r
        ...
        >\r
            <path\r
                d="M228,144v64a12,12,0,0,1-12,12H40a12,12,0,0,1-12-12V144a12,12,0,0,1,24,0v52H204V144a12,12,0,0,1,24,0Zm-108.49,8.49a12,12,0,0,0,17,0l40-40a12,12,0,0,0-17-17L140,115V32a12,12,0,0,0-24,0v83L96.49,95.51a12,12,0,0,0-17,17Z"\r
            />\r
        </svg>\r
        <svg\r
...
function V(a, e) {
	...
	if (!t.find(".download-circle").length) {
		let o = Lampa.Template.get("download-circle", {
			id: a.id,
			status: a.percentDone === 1 ? "complete" : "in-progress",
			progress: 100 * (1 - a.percentDone)
		});
...
function H(a) {
	...
	e.length && e.forEach(t => {
		if (a.percentDone === 1) {
			...
			t.remove(), V(a, o)
		} else t.querySelector(".download-circle__partial_in-progress")?.setAttribute("stroke-dashoffset", `${100 * (1 - a.percentDone)}`)
	})
}

function Z() {
	Lampa.Template.add("download-circle", j), $("body").append(`<style>${Y}</style>`), Lampa.Listener.follow("line", a => {
		if (a.type === "append")
			for (let e of a.items) e?.data?.id && _e(e?.data?.id, e.card)
	})
}
var u = class a {
	static {
	...
	static start() {
		let e = Q[Lampa.Storage.field(C)];
		a.subscription && clearInterval(a.subscription), a.errorCount = 0, a.notified = !1, a.subscription = setInterval(a.tick, e * 1e3)
	}
	static async tick() {
		try {
			let e = await r.getClient().getData();
			if (l.setData(e), $(".d-updatable").length)
	...
	static notifyFirstTime(e) {
		a.notified || (Lampa.Noty.show(e), a.notified = !0)
	}
};
var C = `${i.component}.interval`,
	A = `${i.component}.default-action`,
	b = `${i.component}.server.url`,
	E = `${i.component}.server.login`,
	P = `${i.component}.server.password`,
	k = `${i.component}.server.type`,
	Q = [2, 5, 10, 30, 60, 5 * 60, 15 * 60];
...
function X() {
	Lampa.SettingsApi.addComponent({
		...
		param: {
			...
			type: "select",
			placeholder: "",
			values: ["Open actions menu", "Play", "Resume / Pause download"],
			default: 0
		},
		field: {
...
function L(a) {
	let e = a.find(t => t.startsWith(ee))?.split(":")[1] || "";
	return parseInt(e)
}
...
var S = class {
	...
	async fetchWithAuth(e, t = {}) {
		let o = await fetch(this.url + e, {
			...t,
			credentials: "include"
		});
		return !o.ok && o.status === 403 && (await this.authorize(), o = await fetch(this.url + e, {
			...t,
			credentials: "include"
		})), o
	}
	async authorize() {
		...
		let t = await fetch(this.url + "/api/v2/auth/login", {
			method: "POST",
			body: e,
			credentials: "include"
		});
		if (!t.ok) throw new Error("qBittorrent login failed");
		this.cookie = t.headers.get("set-cookie") || void 0
	}
	async getTorrents() {
		let e = await this.fetchWithAuth("/api/v2/torrents/info");
		if (!e.ok) throw new Error("Failed to get torrents");
		let t = await e.json();
		return this.formatTorrents(t)
	}
	async getData() {
		let e = await this.fetchWithAuth("/api/v2/sync/maindata");
		if (!e.ok) throw new Error("Failed to get qBittorrent info");
		let t = await e.json();
		return {
			torrents: this.formatTorrents(Array.isArray(t.torrents) ? t.torrents : Object.keys(t.torrents).map(o => ({
				...t.torrents[o],
				hash: o
			}))),
			info: {
				freeSpace: t.server_state.free_space_on_disk
			}
		}
	}
	async addTorrent(e, t) {
		...
		if (n.searchParams.delete("dn"), o.append("urls", n.toString()), o.append("tags", g(e.id)), o.append("sequentialDownload", "true"), !(await this.fetchWithAuth("/api/v2/torrents/add", {
				method: "POST",
				body: o
			})).ok) throw new Error("Failed to add torrent")
	}
	async startTorrent(e) {
		let t = new URLSearchParams;
		if (t.append("hashes", String(e.externalId)), !(await this.fetchWithAuth("/api/v2/torrents/start", {
				method: "POST",
				body: t
			})).ok) throw new Error("Failed to start torrents")
	}
	async stopTorrent(e) {
		let t = new URLSearchParams;
		if (t.append("hashes", String(e.externalId)), !(await this.fetchWithAuth("/api/v2/torrents/stop", {
				method: "POST",
				body: t
			})).ok) throw new Error("Failed to stop torrents")
	}
	async hideTorrent(e) {
		let t = new URLSearchParams;
		if (t.append("hashes", String(e.externalId)), t.append("tags", "hide"), !(await this.fetchWithAuth("/api/v2/torrents/addTags", {
				method: "POST",
				body: t
			})).ok) throw new Error("Failed to hide torrent")
	}
	async removeTorrent(e, t = !1) {
		let o = new URLSearchParams;
		if (o.append("hashes", String(e.externalId)), o.append("deleteFiles", t ? "true" : "false"), !(await this.fetchWithAuth("/api/v2/torrents/delete", {
				method: "POST",
				body: o
			})).ok) throw new Error("Failed to remove torrents")
	}
	async getFiles(e) {
		let t = new URLSearchParams;
		t.append("hash", String(e.externalId));
		let o = await this.fetchWithAuth(`/api/v2/torrents/files?${t.toString()}`);
		if (!o.ok) throw new Error(`Failed to get files for torrent ${e.externalId}`);
		return (await o.json()).map(d => ({
			bytesCompleted: Math.floor(d.progress * d.size),
			length: d.size,
			name: d.name,
			begin_piece: d.piece_range?.[0],
			end_piece: d.piece_range?.[1]
		}))
	}
	formatTorrents(e) {
		return e.sort((t, o) => o.added_on - t.added_on).filter(t => !t.tags.includes("hide")).map(t => ({
			id: L(t.tags.split(",")),
			externalId: t.hash,
			name: t.name,
			status: G(t.state),
...
var x = class {
	...
	async POST(e) {
		...
		if (t.status === 409) {
			if (this.sessionId = t.headers.get("X-Transmission-Session-Id"), this.sessionId == null) throw new Error("Can`t authorize to Transmission RPC");
			return this.POST(e)
		}
		if (!t.ok) throw {
			message: `Transmission RPC error: ${t.statusText}`,
			status: t.status
		};
		return await t.json()
	}
...
var I = class {
	constructor(e, t, o) {
		...
		this.login = t;
		this.password = o;
		this.client = new x(e + "/transmission/rpc", t, o)
	}
	async getTorrents() {
		return (await this.client.getTorrents({
			fields: ["id", "name", "status", "percentDone", "sizeWhenDone", "rateDownload", "eta", "labels", "files", "peersConnected", "peersSendingToUs", "trackerStats"]
		})).arguments?.torrents.filter(t => !Array.isArray(t.labels) || t.labels.indexOf("hide") === -1).map(t => {
			let o = 0,
				n = 0;
			return Array.isArray(t.trackerStats) && (o = Math.max(...t.trackerStats.map(d => d.seederCount || 0), 0)), n = t.peersSendingToUs || 0, {
				id: L(t.labels),
				externalId: t.id,
				name: t.name,
				status: M(t.status),
	...
	async addTorrent(e, t) {
		let o = await this.client.addTorrent({
			paused: !1,
			sequential_download: !0,
			filename: t.MagnetUri || t.Link,
			labels: [g(e.id)]
		});
		o.arguments?.["torrent-added"] && await this.client.setTorrent({
			ids: [o.arguments["torrent-added"].id],
			labels: [g(e.id)]
		})
	}
	async startTorrent(e) {
		await this.client.startTorrent({
			ids: [e.externalId]
		})
	}
	async stopTorrent(e) {
		await this.client.stopTorrent({
			ids: [e.externalId]
		})
	}
	async hideTorrent(e) {
...
var r = class a {
	...
	static getClient() {
		if (!this.client) {
			let e = Lampa.Storage.field(b),
				t = e.split(";");
			t.length === 1 && a.buildClient(e), t.length > 1 && a.selectUrl(t)
		}
		return this.client
	}
	static reset() {
	...
	static buildClient(e) {
		let t = Lampa.Storage.field(k) === 1,
			o = Lampa.Storage.field(E),
			n = Lampa.Storage.field(P);
		this.client = t ? new S(e, o, n) : new I(e, o, n)
	}
	static async selectUrl(e) {
		let t = e.map(o => fetch(o + "/ping", {
			cache: "no-cache"
		}).then(n => n.ok ? o : Promise.reject()));
		return new Promise(o => {
			let n = 0,
				d = !1;
			t.forEach(p => p.then(m => {
				d || (d = !0, this.buildClient(m), o())
			}).catch(() => {
				++n === t.length && !d && (this.buildClient(e[0]), o())
			}))
		})
	}
};
var te = i.component + ".torrents.data.views.",
	v = class a {
		static getViews(e) {
			return Lampa.Storage.get(te + e.externalId)
		}
		static rememberView(e, t) {
			let o = a.getViews(e) || {};
			o.last = t, o[t] = !0, Lampa.Storage.set(te + e.externalId, o)
		}
	};
async function ae(a, e, t) {
	let o = r.getClient(),
		n = await o.getFiles(e),
		d = o.url + "/downloads/";
	if (n.length < 1) throw new Error("No files found in torrent");
	if (n.length === 1 && Lampa.Player.play({
			title: t || e.name,
			url: d + n[0].name
		}), n.length > 1) {
		let p = v.getViews(e),
			m = n.map((c, ye) => ({
				title: c.name.split(/[\\/]/).pop() || c.name,
				name: c.name,
				url: d + c.name,
				picked: p[c.name],
				selected: p.last === c.name
			}));
		Lampa.Select.show({
			title: Lampa.Lang.translate("actions.select-file"),
			items: m,
			async onSelect(c) {
				v.rememberView(e, c.name), Lampa.Player.play({
					playlist: m,
					title: t || e.name,
					url: c.url
				}), Lampa.Player.playlist(m), Lampa.Controller.toggle(a)
			},
			onBack: function() {
...
function oe(a) {
	a.status === s.STOPPED ? r.getClient().startTorrent(a) : r.getClient().stopTorrent(a)
}

function _(a, e, t) {
	e = l.ensureMovie(e), Lampa.Select.show({
		...
		items: [{
			...
			title: e.status === s.STOPPED ? Lampa.Lang.translate("actions.resume") : Lampa.Lang.translate("actions.pause"),
			onSelect() {
				oe(e), Lampa.Controller.toggle(a)
			}
		}, {
			title: Lampa.Lang.translate("actions.hide"),
			onSelect() {
				r.getClient().hideTorrent(e), $(`.downloads-tab__item[data-id="${e.id}_${e.externalId}"]`).remove(), Lampa.Controller.toggle(a)
			}
		}, {
			title: Lampa.Lang.translate("actions.delete"),
			subtitle: Lampa.Lang.translate("actions.delete-with-file"),
			onSelect() {
				r.getClient().removeTorrent(e, !0), $(`.downloads-tab__item[data-id="${e.id}_${e.externalId}"]`).remove(), Lampa.Controller.toggle(a)
			}
		}, {
			title: Lampa.Lang.translate("actions.delete-torrent"),
			subtitle: Lampa.Lang.translate("actions.delete-torrent-keep-file"),
			onSelect() {
				r.getClient().removeTorrent(e, !1), $(`.downloads-tab__item[data-id="${e.id}_${e.externalId}"]`).remove(), Lampa.Controller.toggle(a)
			}
		}],
		onBack: function() {
			Lampa.Controller.toggle(a)
...
function y(a, e, t) {
	let o = Lampa.Storage.field(A);
	o == 1 ? ae(a, e, t) : o == 2 ? oe(e) : _(a, e, t)
}
var N = class {
	constructor() {
		this.html = $("<div></div>");
	...
	create() {
		...
		let e = l.getData(),
			t = r.isConnected ? Lampa.Lang.translate("downloads-tab.connected") + " (" + r.getClient().url + ")" : Lampa.Lang.translate("downloads-tab.disconnected"),
			o = $(Lampa.Template.get("downloads-tab", {
				server: t,
				freeSpace: Lampa.Lang.translate("downloads-tab.freespace") + h(e.info.freeSpace)
			}));
		if (!r.isConnected) {
			let n = $('<div class="downloads-tab__item selector" style="width: auto">\u041E\u0431\u043D\u043E\u0432\u0438\u0442\u044C</div>');
			n.on("hover:focus", d => this.scroll.update(d.currentTarget, !0)).on("hover:enter", () => u.start()), o.append(n)
		}
		e.torrents.forEach(n => {
			let d = w(n),
				p = $(Lampa.Template.get("downloads-row", {
					...d,
					icon: f
				})).on("hover:focus", m => this.scroll.update(m.currentTarget, !0)).on("hover:enter", () => y("downloads-tab", n)).on("hover:long", () => _("downloads-tab", n));
			o.append(p)
	...
	start() {
		Lampa.Controller.add("downloads-tab", {
			toggle: () => {
				Lampa.Controller.collectionSet(this.scroll.render()), Lampa.Controller.collectionFocus(this.lastFocusedElement ?? !1, this.scroll.render())
			},
			left: () => Navigator.canmove("left") ? Navigator.move("left") : Lampa.Controller.toggle("menu"),
			right: () => Navigator.move("right"),
			up: () => {
				Navigator.canmove("up") ? Navigator.move("up") : Lampa.Controller.toggle("head"), this.lastFocusedElement = Navigator.getFocusedElement()
			},
			down: () => {
				Navigator.canmove("down") && Navigator.move("down"), this.lastFocusedElement = Navigator.getFocusedElement()
			},
	...
	stop() {}
	destroy() {
		this.scroll.destroy(), this.html.remove()
	}
};
...
function J(a) {
	...
	t.length && (t.removeClass("downloading completed paused").addClass(e.status), t.find(".downloads-tab__title span").text(e.fileName), t.find(".downloads-tab__speed span").text(e.speed), t.find(".downloads-tab__meta-eta span").text(e.eta), t.find(".downloads-tab__progress-fill").css("width", e.percent), t.find(".downloads-tab__meta-percent").text(e.percent), t.find(".downloads-tab__meta-downloaded").text(e.downloadedSize), t.find(".downloads-tab__meta-total").text(e.totalSize), t.find(".downloads-tab__meta-seeders").text(e.seeders))
}

function ne() {
	Lampa.Template.add("menu-button", F), Lampa.Template.add("downloads-row", z), Lampa.Template.add("downloads-tab", O), $("body").append(`<style>${U}</style>`), Lampa.Component.add("downloads-tab", N);
	let a = Lampa.Lang.translate("downloads"),
		e = $(Lampa.Template.get("menu-button", {
			icon: f,
			text: a
		}));
...
var re = `<div class="full-start__button selector button--download">\r
    {icon}\r
    <span>{text}</span>\r
</div>`;

function Te(a) {
	let e = $(Lampa.Template.get("download-button", {
		icon: f,
...
function ie() {
	Lampa.Template.add("download-button", re), Lampa.Component.add("torrents-download", Lampa.Component.get("torrents")), Lampa.Listener.follow("full", a => {
		if (a.type === "complite") {
			let e = a.data;
			Te(e)
		}
		...
		a.type === "render" && e.component === "torrents-download" && ($(a.item).off("hover:enter"), $(a.item).on("hover:enter", async () => {
			await r.getClient().addTorrent(e.movie, a.element), Lampa.Activity.back();
			let o = (await r.getClient().getTorrents()).find(n => n.id === e.movie.id);
			D(o, e.movie)
		}))
	})
}
...
function de() {
	window.plugin_transmission_ready = !0, Lampa.Manifest.plugins = i, Lampa.Lang.add(se), X(), ie(), K(), ne(), Z(), Lampa.Storage.field(b) && u.start()
}
window.plugin_transmission_ready || (window.appready ? de() : Lampa.Listener.follow("app", function(a) {
a.type === "ready" && de()
}));
})();
